                ...
                @foreach($brands->take(6) as $brand)
                    <div class="col-md-2 col-6 text-center">
                        <div class="brand-card">
                            @if($brand->logo)
                                <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="img-fluid mb-2" style="max-height: 60px; filter: drop-shadow(0 2px 8px #6366f1aa);">
                            @else
                                <i class="fas fa-industry fa-3x mb-2"></i>
                            @endif
                            <div><h6 class="mb-0">{{ $brand->name }}</h6></div>
                        </div>
                    </div>
                @endforeach
            </div>
                ...
                @foreach($latestNews as $news)
                    <div class="col-md-4">
                        <div class="card news-card h-100">
                            <img src="{{ $news->image ? asset('storage/' . $news->image) : 'https://via.placeholder.com/400x250/cccccc/666666?text=News' }}" 
                                 class="card-img-top" alt="{{ $news->title }}" style="height: 200px; object-fit: cover;">
                            <div class="card-body">
                                <h6 class="card-title">{{ $news->title }}</h6>
                                <p class="card-text text-muted">{{ Str::limit($news->summary ?? $news->content, 120) }}</p>
                                <div class="d-flex justify-content-between align-items-center">
                                    <small class="text-muted">{{ $news->created_at ? $news->created_at->format('d/m/Y') : 'N/A' }}</small>
