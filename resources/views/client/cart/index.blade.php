    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-8 mb-4 mb-lg-0">
                ...
                <div class="cart-item-card" data-cart-item-id="{{ $item->id }}" data-product-id="{{ $item->product->id }}">
                    ...
                    <div class="cart-item-info">
                        ...
                        <div class="stock-info {{ $item->low_stock ? 'low-stock' : '' }}">
                            Số lượng tồn: {{ $item->product->stock }}
                            @if($item->low_stock)
                                <span class="low-stock-badge">(Sắp hết hàng)</span>
                            @endif
                        </div>
                    </div>
                    <div class="cart-item-price price-single" data-price="{{ $item->price }}">{{ number_format($item->price, 0, ',', '.') }}đ</div>
                    ...
                    <div class="cart-item-total" data-total="{{ $item->price * $item->quantity }}">{{ number_format($item->price * $item->quantity, 0, ',', '.') }}đ</div>
                    <div class="cart-item-remove">
                        <form action="{{ route('client.cart.remove', $item->id) }}" method="POST" class="remove-form">
                            @csrf
                            @method('DELETE')
            ...
            <div class="col-lg-4">
                <div class="cart-summary">
                    ...
                    <a href="{{ route('client.checkout') }}" class="btn btn-primary btn-block">Tiến hành thanh toán</a>
                </div>
            </div>
            @endif
            @if(!$cartItems || !$cartItems->count())
